...
# Fetched from https://github.com/NixOS/nixpkgs/blob/master/pkgs/applications/editors/neovim/utils.nix
# Added `extraRuntimeDeps` for language servers.
let
  ...
  makeNeovimConfig =
    ...
    , customRC ? ""
    # Extra packages to add to the wrapped output.
    , extraRuntimeDeps ? []
    ...
    let
      ...
      makeWrapperArgs =
        let
          ...
          extraBinPath = lib.makeBinPath extraRuntimeDeps;
